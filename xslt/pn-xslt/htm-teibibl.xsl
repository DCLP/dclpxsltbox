...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  xmlns:pi="http://papyri.info/ns" xmlns:dcterms="http://purl.org/dc/terms/"
  ...
  <xsl:template match="t:bibl">
    <xsl:if test="t:seg[@type='original' and @resp='#BP']">
      <div class="biblio">
        ...
        <p>
          <xsl:apply-templates select="t:seg[@type='original' and @subtype='index']"/>
    ...
    <div class="biblio">
      ...
      <p>
        ...
        <xsl:text> [</xsl:text>
        <a href="{t:idno[@type='pi']}/source">xml</a>
        <xsl:text>] [</xsl:text>
        <a class="button" id="editbibl"
          href="/editor/publications/create_from_identifier/papyri.info/biblio/{t:idno[@type='pi']}"
          >edit</a>
        <xsl:text>]</xsl:text>
      </p>
    </div>
  ...
  <xsl:template name="buildCitation">
    <xsl:variable name="mainWork"
      select="pi:get-docs(concat(t:relatedItem[@type='appearsIn' and t:bibl/t:ptr/@target != 'http://papyri.info/biblio/0'][1]//t:ptr/@target, '/source'), 'xml')"/>
    ...
    <xsl:variable name="mainTitle"><xsl:choose>
        <xsl:when test="(@type='article' or @type='review') and $mainWork//*"><xsl:apply-templates
            select="$mainWork/t:bibl" mode="mainTitle"/></xsl:when>
        <xsl:otherwise><i><xsl:value-of select="t:title[@type='main']"/></i><xsl:if
            test="t:title[@type='short']"> (<i><xsl:value-of select="t:title[@type='short']"
            /></i>)</xsl:if></xsl:otherwise>
      </xsl:choose></xsl:variable>
    <xsl:variable name="pubInfo"><xsl:call-template name="pubInfo"><xsl:with-param name="main"
          select="$mainWork"/></xsl:call-template></xsl:variable>
    <b><xsl:value-of select="t:idno[@type='pi']"/>. </b>
    ...
    <xsl:if test="t:relatedItem[@type='appearsIn']"><xsl:if test="t:title">"</xsl:if><xsl:copy-of
        select="$articleTitle"/><xsl:if test="@subtype='journal'">,</xsl:if><xsl:if test="t:title"
        >"</xsl:if>
    </xsl:if>
    <xsl:copy-of select="$mainTitle"/><xsl:if test="string-length($pubInfo) > 0">,
      </xsl:if><xsl:value-of select="$pubInfo"/>. </xsl:template>

  <xsl:template name="author">
    <xsl:for-each select="t:author">
      <xsl:if test="count(../t:author) > 1 and position() = last()">
      ...
      <xsl:if test="(position() != last()) and (count(../t:author) > 2)">
        <xsl:text>, </xsl:text>
      </xsl:if>
    </xsl:for-each>
  </xsl:template>
  ...
  <xsl:template name="articleTitle">
    <xsl:choose>
      ...
      <xsl:when test="@type='review'">Review of <xsl:for-each
          select="t:relatedItem[@type='reviews']"><xsl:for-each
            select="pi:get-docs(concat(.//t:ptr/@target, '/source'), 'xml')/t:bibl"><a
              href="/biblio/{t:idno[@type='pi']}/"><xsl:call-template name="buildCitation"
            /></a></xsl:for-each></xsl:for-each><xsl:text>, </xsl:text></xsl:when>
    </xsl:choose>
  </xsl:template>
      ...
      <a href="/biblio/{t:idno[@type='pi']}/">
        <xsl:choose>
          ...
          <xsl:when test="t:title[@level='j']">
            <xsl:choose>
              <xsl:when test="t:title[@level='j' and @type='short']">
                <xsl:value-of select="t:title[@level='j' and @type='short']"/>
              </xsl:when>
              <xsl:otherwise>
                <xsl:value-of select="t:title[@level='j' and @type='main']"/>
              </xsl:otherwise>
            </xsl:choose>
          </xsl:when>
        </xsl:choose>
  ...
  <xsl:template name="editor">
    <xsl:for-each select="t:editor">
      ...
      <xsl:value-of select="."/>
      <xsl:if test="count(../t:editor) > 2 and position() != last()">, </xsl:if>
    </xsl:for-each>
    <xsl:if test="t:editor"> ed<xsl:if test="count(t:editor) > 1">s</xsl:if>.</xsl:if>
  </xsl:template>
    ...
    <xsl:choose>
      ...
      <xsl:when test="$main//t:title[@level='m']">
        <xsl:if test="t:series">
          ...
          <xsl:if test="t:series/t:biblScope[@type='volume']"> vol. <xsl:value-of
              select="t:series/t:biblScope[@type='volume']"/></xsl:if>
        </xsl:if>
        <xsl:if test="t:pubPlace or t:date">(<xsl:if test="$main//t:pubPlace"><xsl:value-of
              select="$main//t:pubPlace"/><xsl:text> </xsl:text></xsl:if><xsl:value-of
                select="$main//t:date"/>)</xsl:if><xsl:text> </xsl:text>
      ...
      <!-- journal -->
      <xsl:when test="t:title[@level='j']">
        <xsl:if test="t:publisher or t:date">(<xsl:if test="t:publisher"><xsl:value-of
              select="t:publisher"/><xsl:text> </xsl:text></xsl:if><xsl:value-of select="t:date"
          />)</xsl:if>
      </xsl:when>
      <!-- book -->
      <xsl:when test="t:title[@level='m']">(<xsl:if test="t:pubPlace"><xsl:value-of
            select="t:pubPlace"/><xsl:text> </xsl:text></xsl:if><xsl:value-of select="t:date"
        />)</xsl:when>
      <xsl:when test="t:pubPlace or t:date">(<xsl:if test="t:pubPlace"><xsl:value-of
            select="t:pubPlace"/><xsl:text> </xsl:text></xsl:if><xsl:value-of select="t:date"
        />)</xsl:when>
    </xsl:choose>
  </xsl:template>
  ...
  <xsl:template match="t:seg[@type='original']">
    <xsl:choose>
      ...
      <xsl:when test="@subtype='publication'">Publication: <xsl:value-of select="."
        /><br/></xsl:when>
      <xsl:when test="@subtype='sbSeg'">S.B. &amp; S.E.G.: <xsl:value-of select="."
        /><br/></xsl:when>
  ...
  <xsl:template match="t:note[@resp='#BP']"> Résumé: <xsl:value-of select="."/><br/>
  </xsl:template>

  <xsl:template name="bpId">
    <xsl:choose>
      <xsl:when test="string(//t:idno[@type='bp'])"> No: <xsl:value-of select="//t:idno[@type='bp']"
        /><br/>
      </xsl:when>
      <xsl:when test="string(//t:idno[@type='bp_old'])"> Ancien No: <xsl:value-of
          select="//t:idno[@type='bp_old']"/><br/>
      </xsl:when>
      <xsl:otherwise> There is no bp id available </xsl:otherwise>
    </xsl:choose>
  </xsl:template>
          ...
          <p>
            <xsl:call-template name="relatedArticleRecord">
              <xsl:with-param name="series"
                select="normalize-space($relatedArticle/t:title[@level='s'][@type='short'])"/>
              <xsl:with-param name="volume"
                select="normalize-space($relatedArticle/t:biblScope[@type='vol'])"/>
              <xsl:with-param name="number"
                select="normalize-space($relatedArticle/t:biblScope[@type='num'])"/>
              <xsl:with-param name="ddbId"
                select="normalize-space($relatedArticle/t:idno[@type='ddb'])"/>
              <xsl:with-param name="inventory"
                select="normalize-space($relatedArticle/t:idno[@type='invNo'])"/>
            </xsl:call-template>
          </p>
        </xsl:otherwise>
      </xsl:choose>
  ...
  <xsl:function name="pi:checkFile">
    ...
    <xsl:variable name="test"
      select="doc-available(pi:get-filename(concat($link, '/source'), 'xml'))"/>
    <xsl:if test="$test">
